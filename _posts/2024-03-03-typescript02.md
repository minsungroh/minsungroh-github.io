---
title: TypeScript 환경 구성
author: happywerrior
date: 2024-03-03 13:29:00 +0900
...
### TypeScript 환경 구성
<span class='ts-09'>`TypeScript`를 Project에 사용하려면 `NodeJs`가 설치되어야 한다. 그리고 `NPM`을 통해 개발에 필요한 모듈을 설치/관리 해야 한다. `NPM`을 사용해 `typeScript 개발 모듈`을 설치해 봅니다.</span>
```console
# Global install
$ npm i -g typescript
...
### TypeScript CLI
<span class='ts-09'>`TypeScript`는 앞서 이야기한대로 `.ts` 확장를 가집니다.</span>
...
### Compile 설정
#### tsconfig.json
<span class='ts-09'>매번 `TypeScript` 컴파일 명령어를 입력할 때마다 옵션을 추가/제거 하는건 매우 불편하기 때문에, 보다 쉽게 사용하려면 `컴파일 설정 파일(tsconfig.json)`을 만들어 사용하는게 편리합</span>
<span class='ts-09'>기본적으로 `tsconfig.json`파일은 `root directory`에 설정</span>
<span class='ts-09'>해당 파일에는 Project를 Compile 하는데 필요한 Root File, Compiler Option 을 설정할 수 있다.</span>
...
### include / exclude 설정
<span class='ts-09'>Compile에 포함할 경로/File을 설정하거나, 제외할 수도 있다.</span>
- <span class='ts-08'>* : 무조건 모든 문자와 일치(디렉토리 분리 기호 제외)</span>
- <span class='ts-08'>? : 1개의 모든 문자와 일치(디렉토리 분리 기호 제외)</span>
- <span class='ts-08'>**/ : 모든 하위 디렉토리 포함</span>
